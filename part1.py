...
class Tree(object):
    ...
    def entropy(Y):
        ...
        e = 0
        for i in lable:
            e = -( (list(Y).count(i)/array_len)*math.log(list(Y).count(i)/array_len,2)  )
            e += e
    ...
    def conditional_entropy(Y,X):
        ...
        conditional_y = []
        for i in lable:
            x_index = [index for (index, vaule) in enumerate(X) if vaule == i]

            for ii in x_index:
                conditional_y.append(Y[ii])
            ce += Tree.entropy(conditional_y)
    ...
    def best_attribute(X,Y):
        ...
        for a in range (len(X)):
            gain_dict[a] = (Tree.information_gain(Y,list(X[a]) ))

        biggest = 0
        for b in list(gain_dict.values()):
            if b > biggest:
                biggest = b
        def get_key(dict, vaule):
            return[k for k, v in dict.items() if v == vaule]
        key_list = get_key(gain_dict,biggest)
        i = min(key_list)
        #########################################
        return i
    ...
    def split(X,Y,i):
        ...
        len_x = len(X)
        attri_x = X[i]
        lable = list((set(attri_x)))
        C = {}
        for i in lable:
            x_index = [index for (index, vaule) in list(enumerate(attri_x)) if vaule == i]  # x的index位置
            x_new = []
            ...
            y_matrix = np.array([Y[c] for c in x_index])
            C[i] = Node(X=x_matrix, Y=y_matrix)
